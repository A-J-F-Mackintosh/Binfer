...
Usage: Binfer_prep.py -v <STR> -b <STR> -g <STR> -d <INT> -D <INT> -w <INT> (-1 <STR> | -o STR>) (-2 <STR> | -t <STR) [-h -a <INT> -f <INT>]
  ...
  [Options]
    ...
    -f, --fold_spectra <INT>                    Fold the SFS rather than using an outgroup or ancestral allele field. If True, args for taxon_2 and are ignored. [default: 0]
...
if __name__ == '__main__':
	...
	fold = int(args["--fold_spectra"])
	if fold == 1: # fold takes priority
		UAA = 0
					...
					if UAA == 0 and fold == 0:
						...
						if len(alleles_taxon_1) >= downsample_1: # enough genomes?
							...
							if len(set(alleles_taxon_1)) == 2: # check if biallelic
								...
								all_alleles = [ref_base] + alt_base.split(",")

								# assign derived and ancestral arbitrarily and fold later							
